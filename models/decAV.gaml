...
//	create a list of people that are near the car 
...
//	reflex to update the path to follow then the car is wandering
...
//	reset variables before going back to a wandering state
...
//	lookm for new passengers on the road and check if the constraints are respected in order to actually pick them up
	reflex add_on_road when: !empty(people_near) and (state='moving') and length(passenger)< max_passengers{
		...
		if !empty(possible_pass){
			...
			if !empty(people_on_road){
				...
				write "addition "+((decAV index_of self)<10 ? "0": "") + string(decAV index_of self) + ", "+ n_travel+ ", "+n_addition+", "+ stats_first_path_time+", "+ total_added_people+", "+ total_available_people + ", "+ initial_stops + ", "+ dest_length + ", "+ length(ordered_or_dest);
			}
		}
		state<-'moving';
		people_near<-nil;
